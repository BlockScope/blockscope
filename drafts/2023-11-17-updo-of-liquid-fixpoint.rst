---
title: How to Updo Liquid FixPoint
subtitle: Walkthrough of converting Liquid Fixpoint to use Updo
tags: haskell, build
---
...
Converting Liquid Fixpoint
==========================

Before the conversion[#]_, there are Stack and Cabal projects. The resolver is a
``ghc-9.6.3`` nightly. Both projects build with ``ghc-9.6.3``.
...
We need to add some constraints to packages that stackage doesn't have versions for.
    ...
    .. code-block:: bash
      ...
      Error: [S-4804]
            ...
            In the dependencies for liquid-fixpoint-0.9.2.5:
            ...
            Some different approaches to resolving some or all of this:
              ...
              * Recommended action: try adding the following to your extra-deps in /.../liquid-fixpoint/stack.yaml
                (project-level configuration):
                
                - smtlib-backends-0.3@sha256:917d88540a9ede7beedbe2ed13b492acddbce394d30ccf5d0ef4f4fba9aa2c12,1157
                - smtlib-backends-process-0.3@sha256:d4d7d02859383e0a43db2d8ce7ef01deffe1bcd356b2ff8626925c3a1c8db922,1600

These recommendations for package versions have a ``@sha256:`` suffix but Updo
doesn't work with that. Let's add those constraints:

    .. code-block:: dhall

      [ { dep = "smtlib-backends", ver ="0.3" }
    ...
    .. code-block:: bash
      ...
      Error: [S-4804]
            ...
            In the dependencies for smtlib-backends-0.3:
              * base must match >=4.14 && <4.18,
                but base-4.18.1.0 is in the Stack configuration (latest matching version is 4.17.2.0).
            needed due to liquid-fixpoint-0.9.2.5 -> smtlib-backends-0.3
            
            In the dependencies for smtlib-backends-process-0.3:
              * base must match >=4.14 && <4.18,
                but base-4.18.1.0 is in the Stack configuration (latest matching version is 4.17.2.0).
            needed due to liquid-fixpoint-0.9.2.5 -> smtlib-backends-process-0.3
            
            Some different approaches to resolving some or all of this:
            
              * To ignore all version constraints and build anyway,
                in /.../.stack/config.yaml (global configuration) or
                /.../liquid-fixpoint/stack.yaml (project-level configuration), set allow-newer: true.
              
              * To ignore certain version constraints and build anyway, also add
                these package names under allow-newer-deps: smtlib-backends and
                smtlib-backends-process.
              
              * Build requires unattainable version of the base package. Since
                base is a part of GHC, you most likely need to use a different
                GHC version with the matching base.

That doesn't work but we can pick up a revision that relaxes that upper bound on base.
