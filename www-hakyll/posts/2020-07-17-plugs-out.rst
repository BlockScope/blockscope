...
Wiring Diagram
--------------
There's more than one way to wire up a plugin.
...
by itself. The ``test-undefined-*-carefree`` test suites with suffix have
these mains. The ones without the ``-carefree`` suffix don't. They have the
``a`` from ``undefined`` mains.
